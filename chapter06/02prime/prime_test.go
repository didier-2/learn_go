...
func TestHalloGoroutine(t *testing.T) {
	go fmt.Println("halle goroutine")
}

func TestHalloGoroutine2(t *testing.T) {
	go fmt.Println("halle goroutine")
	time.Sleep(1 * time.Second)
}
func TestHalloGoroutine3(t *testing.T) {
	go func() {
		for i := 0; i <= 5; i++ {
			fmt.Println(i)
			time.Sleep(1 * time.Second)
...
func TestForeverGoroutine(t *testing.T) {
	go func() {
		for {
			time.Sleep(1 * time.Second)
			go func() {
				fmt.Println("启动新的goroutine了", time.Now())
