...
func distributeLB(ctx context.Context, w *sync.WaitGroup) {
	...
	for {
		select {
		case <-ctx.Done():
			fmt.Println("上下文结束，删除已经创建的负载均衡器")
			return
		default:
		}
		fmt.Println("部署负载均衡器")
...
func verifyFunction(ctx context.Context, wg *sync.WaitGroup) {
	...
	for i := 0; i < 3; i++ {
		...
		fmt.Println("开始验证服务")
		time.Sleep(100 * time.Millisecond) //用来替换验证部分的环节，比如服务调用，服务模拟等
		fmt.Println("服务验证完成")
		if i <= 1 {
			fmt.Println("服务未完成，重试")
			continue
		}
		break
	}
